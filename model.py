...
class ResNet:
    ...
    def residual_module(data, K, stride, chanDim, red=False, reg=0.0001, bnEps=2e-5, bnMom=0.9):
        ...
        conv3 = Conv2D(int(K), (1, 1), use_bias=False, kernel_regularizer=l2(reg))(act3)
    ...
    def build(width, height, depth, classes, stages, filters, reg=0.0001, bnEps=2e-5, bnMom=0.9, dataset="cifar"):
        ...
        #check if we are using the Tiny ImageNet dataset
        ...
        for i in range(0, len(stages)):
            #initialize the stride, then apply a residual module used to reduce the spatial size of input volume
